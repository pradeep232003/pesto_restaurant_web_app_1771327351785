...
4. **Sous Vide** — `/jkhive/sous-vide` · backend `/api/admin/sous-vide` · collection `sousvide_records` · **4-step iOS wizard** matching IMG_6738–IMG_6742: (1) Pick item from shared catalog, (2) Pre-cooked / Raw card chooser, (3) wheel-picker batch count 1–99, (4) water-bath temperature gauge 45–100 °C with ticks 45/56/67/78/89/100 (green when ≥ min-temp), (5) duration twin-wheel hours 0–24 + minutes 0–59 → "Start sous vide". Pass logic: `bath_temp ≥ 63 °C (pre-cooked)` or `≥ 54 °C (raw)` AND total duration > 0. Schema: `raw_or_cooked`, `batch_count`, `bath_temp`, `min_temp`, `duration_hours`, `duration_minutes`, `duration_total_minutes`, `temp_pass`, `time_pass`, `passed`.
