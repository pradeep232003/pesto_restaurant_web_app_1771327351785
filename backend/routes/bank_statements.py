...
def _load_invoices(location_id: str) -> List[dict]:
    ...
    try:
        rows = list(invoices_col.find(
            {"location_id": location_id},
            {
                ...
                "invoice_date": 1, "total": 1, "grand_total": 1, "vat": 1,
            },
        ).limit(2000))
    except Exception:
        return []
    ...
    for r in rows:
        ...
        try:
            ...
            amt = 0.0
        # VAT — captured by the invoice scanner (`vat` field). Legacy
        # rows may lack it entirely so we default to 0.
        try:
...
def _enrich_with_invoices(txns: List[dict], location_id: str) -> int:
    ...
    for t in expense_txns:
        ...
        t["matched_invoice_vat"] = inv.get("vat", 0.0)
...
async def aggregate_xlsx(
    ...
    total_vat = round(sum(
        float(t.get("matched_invoice_vat") or 0)
        for t in expense_txns if t.get("matched_invoice_id")
    ), 2)
    matched_vat_rows = sum(1 for t in expense_txns if t.get("matched_invoice_id"))
    ...
    summary_rows = [
        ...
        ("Total VAT (from linked invoices)", total_vat),
        ("Expense rows with invoice", f"{matched_vat_rows} of {len(expense_txns)}"),
...
def _build_split_xlsx(income_txns: list, expense_txns: list, summary_rows: list) -> io.BytesIO:
    ...
    def _write_sheet(ws, title, txns, fill):
        ...
        # supplier name and a VAT column populated from the invoice.
        ...
        widths = [14, 56, 18] + ([32] if is_expenses else []) + [28, 14] + ([12] if is_expenses else []) + ([28, 22] if aggregated else [])
        ...
        for r, t in enumerate(txns, start=2):
            ...
            col += 1
            if is_expenses:
                # VAT is only meaningful when there's a real invoice link;
                # blank cells (rather than £0.00) prevent the totals row
                # from implying we accounted for VAT on unlinked rows.
                vat_value = float(t.get("matched_invoice_vat") or 0) if t.get("matched_invoice_id") else None
                vat_cell = ws.cell(row=r, column=col, value=vat_value)
        ...
        if txns:
            ...
            # VAT total (expenses only) — sums only the linked-invoice rows.
            if is_expenses:
                vat_total = sum(
                    float(t.get("matched_invoice_vat") or 0)
                    for t in txns if t.get("matched_invoice_id")
                )
                vt = ws.cell(row=total_row, column=amt_col + 1, value=vat_total)
                vt.font = Font(bold=True)
