...
import { Droplet, Plus, Trash2, Check, X, ArrowLeft, Download } from 'lucide-react';
...
const AdminLegionella = () => {
  ...
  const fetchEntries = async () => {
    setLoading(true);
    try { setEntries(await api.adminListLegionella({ location_id: selectedLocation, start_date: today, end_date: today })); }
  ...
  const fetchHistory = async () => {
    ...
    try { setHistory(await api.adminListLegionella({ location_id: selectedLocation, start_date: historyStart, end_date: historyEnd })); }
    catch (err) { alert('Failed: ' + err.message); }
    finally { setHistoryLoading(false); }
  };

  const handleSave = async () => {
    if (!form.location_of_test) return;
    setSaving(true);
    try {
      await api.adminCreateLegionella({
        location_id: selectedLocation,
        date: form.date,
        test_time: form.test_time,
        hot_water_temp: form.hot_water_temp === '' ? null : parseFloat(form.hot_water_temp),
        cold_water_temp: form.cold_water_temp === '' ? null : parseFloat(form.cold_water_temp),
        name: form.name,
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.adminDeleteLegionella(id); if (activeTab === 'today') fetchEntries(); else fetchHistory(); }
    catch (err) { alert('Failed: ' + err.message); }
  };
  ...
  const handleDownload = () => {
    ...
    const rows = history.map(e => ({
      Date: e.date, Time: e.test_time || '', 'Test Location': e.location_of_test,
      'Hot °C': e.hot_water_temp, 'Cold °C': e.cold_water_temp,
      Pass: e.passed ? 'Yes' : 'No', Name: e.name || '', Initials: e.initials || '',
      'Action Taken': e.action_taken || '', 'Logged by': e.created_by_name || e.created_by,
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Legionella');
    XLSX.writeFile(wb, `legionella_${locName}_${historyStart}_to_${historyEnd}.xlsx`);
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto" data-testid="admin-legionella-page">
      ...
      <div className="mb-5">
        <h1 className="text-xl sm:text-2xl font-semibold tracking-tight" style={{ color: '#1D1D1F', ...font }}>Legionella Water Testing</h1>
        <p className="text-xs sm:text-sm mt-1" style={{ color: '#86868B' }}>Hot water &gt; 50°C · Cold water &lt; 20°C · Test alternative taps each week.</p>
      </div>

      <div className="mb-4">
        <select data-testid="legionella-location" value={selectedLocation} onChange={e => setSelectedLocation(e.target.value)}
      ...
      {!selectedLocation ? (
        <div className="text-center py-16 rounded-2xl" style={{ background: '#FFFFFF' }}>
          <Droplet size={32} className="mx-auto mb-3" style={{ color: '#C7C7CC' }} />
        ...
        <>
          {isAdmin && (
            <div className="flex gap-1 mb-5 p-1 rounded-xl" style={{ background: '#E8E8ED' }}>
              {[{k:'today',l:'Today'},{k:'history',l:'History'}].map(t => (
                <button key={t.k} data-testid={`legionella-tab-${t.k}`} onClick={() => setActiveTab(t.k)}
          ...
          {activeTab === 'today' && (
            <button data-testid="add-legionella-btn" onClick={() => setShowForm(!showForm)}
          ...
          {activeTab === 'today' && showForm && (
            <div className="p-4 rounded-2xl mb-4 space-y-3" style={{ background: '#FFFFFF' }}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <input type="time" value={form.test_time} onChange={e => setForm({ ...form, test_time: e.target.value })} className="px-3 py-2.5 rounded-xl text-sm border-0 outline-none" style={inputStyle} />
              </div>
              <input data-testid="test-location" placeholder="Location of test (e.g. Kitchen tap, Bathroom)" value={form.location_of_test} onChange={e => setForm({ ...form, location_of_test: e.target.value })} className="w-full px-3 py-2.5 rounded-xl text-sm border-0 outline-none" style={inputStyle} />
              <div className="grid grid-cols-2 gap-3">
                <input data-testid="hot-water" type="number" step="0.1" placeholder="Hot °C (&gt; 50)" value={form.hot_water_temp} onChange={e => setForm({ ...form, hot_water_temp: e.target.value })} className="px-3 py-2.5 rounded-xl text-sm border-0 outline-none" style={inputStyle} />
          ...
          {activeTab === 'history' && isAdmin && (
            <div className="p-4 rounded-2xl mb-4 space-y-3" style={{ background: '#FFFFFF' }}>
              ...
              <button data-testid="download-legionella-btn" disabled={!history.length} onClick={handleDownload}
          ...
          {listLoading ? (
            ...
            <div className="space-y-2">
              {list.map(e => (
                <div key={e.id} data-testid={`legionella-entry-${e.id}`} className="p-4 rounded-2xl flex items-start gap-3" style={{ background: '#FFFFFF' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium" style={{ color: '#1D1D1F', ...font }}>{e.location_of_test}</p>
                    <p className="text-[11px]" style={{ color: '#86868B' }}>
                      {e.date}{e.test_time ? ` · ${e.test_time}` : ''}
                      {e.hot_water_temp != null && ` · Hot ${e.hot_water_temp}°C`}
                      {e.cold_water_temp != null && ` · Cold ${e.cold_water_temp}°C`}
...
export default AdminLegionella;
