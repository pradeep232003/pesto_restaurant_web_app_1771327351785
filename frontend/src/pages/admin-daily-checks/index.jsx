...
const AdminDailyChecks = () => {
  ...
  const loadExisting = async () => {
    setLoading(true);
    try {
      const d = await api.adminGetDailyCheck(selectedLocation, entryDate);
      if (d) {
        setChecks(d.checks || {});
        setNote(d.note || '');
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await api.adminSubmitDailyCheck({ location_id: selectedLocation, date: entryDate, checks, note });
  ...
  const fetchHistory = async () => {
    ...
    try { setHistory(await api.adminGetDailyChecksHistory({ location_id: selectedLocation || undefined })); }
    catch {} finally { setHistoryLoading(false); }
  };

  const fetchGrid = async () => {
    setGridLoading(true);
    try { setGridData(await api.adminGetDailyChecksCompletion(gridMonth)); }
  ...
  return (
    <div className={isJkhive ? "px-4 sm:px-6 lg:px-8 pt-2 pb-4 max-w-5xl mx-auto" : "p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto"} data-testid="admin-daily-checks-page">
      {isJkhive ? (
      ...
      <div className="mb-5">
        {!isJkhive && (
          <h1 className="text-xl sm:text-2xl font-semibold tracking-tight" style={{ color: '#1D1D1F', ...font }}>Daily Checks</h1>
        )}
        <p className="text-xs sm:text-sm mt-1" style={{ color: '#86868B' }}>Opening checks — complete before serving</p>
      </div>

      <div className="mb-4">
        <select data-testid="checks-location" value={selectedLocation} onChange={e => setSelectedLocation(e.target.value)}
        ...
        <>
          ...
          {activeTab === 'check' && (
            <div>
              ...
              {loading ? (
                ...
                <div className="space-y-2">
                  {items.map((item, i) => {
                    ...
                    return (
                      <button key={item.id} data-testid={`check-${item.id}`}
              ...
              <button data-testid="save-checks-btn" onClick={handleSave} disabled={saving || !selectedLocation}
          ...
          {activeTab === 'history' && isAdmin && (
            <div>
              {historyLoading ? (
                ...
                <div className="text-center py-16 rounded-2xl" style={{ background: '#FFFFFF' }}>
                  <ClipboardCheck size={32} className="mx-auto mb-3" style={{ color: '#C7C7CC' }} />
                  <p className="text-sm" style={{ color: '#86868B', ...font }}>No daily checks recorded yet.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {history.map(entry => {
