...
const SousVideHome = () => {
  ...
  const load = () => {
    ...
    setLoading(true);
    api.sousVideList(adminLocationId).then(d => setRows(d || [])).finally(() => setLoading(false));
  };
  ...
  return (
    <div style={{ paddingBottom: 110, fontFamily: 'Outfit, sans-serif' }} data-testid="sous-vide-home">
      ...
      {!loading && rows.length === 0 && (
        <p style={{ color: '#86868B', textAlign: 'center', padding: 24 }}>No sous-vide records yet. Tap + to add one.</p>
      )}
      ...
      <div style={{ position: 'fixed', left: 0, right: 0, bottom: 64, zIndex: 5 }}>
        <button data-testid="sous-vide-add-btn"
          ...
          <Plus size={20} strokeWidth={2.6} /> Add Sous Vide Cook
        </button>
      </div>
    </div>
  );
