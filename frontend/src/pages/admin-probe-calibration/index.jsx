...
import { Gauge, Plus, Trash2, Check, X } from 'lucide-react';
...
const AdminProbeCalibration = () => {
  ...
  const fetchEntries = async () => {
    setLoading(true);
    try { setEntries(await api.adminListProbeCalibration({ location_id: selectedLocation, start_date: startDate, end_date: endDate })); }
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.adminDeleteProbeCalibration(id); await fetchEntries(); }
    catch (err) { alert('Failed: ' + err.message); }
  };
      ...
      {!selectedLocation ? (
        ...
        <>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 rounded-2xl" style={{ background: '#FFFFFF' }}>
              <Gauge size={32} className="mx-auto mb-3" style={{ color: '#C7C7CC' }} />
              <p className="text-sm" style={{ color: '#86868B', ...font }}>No calibration tests in this range.</p>
            </div>
          ) : (
