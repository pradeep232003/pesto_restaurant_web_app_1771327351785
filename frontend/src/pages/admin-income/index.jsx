...
import { TrendingUp, Plus, Trash2, Filter, Calendar } from 'lucide-react';
      ...
      {showForm && (
        <form onSubmit={handleSave} className="p-4 rounded-2xl mb-5 space-y-3" style={{ background: '#FFFFFF' }}>
          ...
          <div>
            ...
            <input data-testid="income-description" type="text" placeholder="e.g. Catering event, Private hire" value={description} onChange={e => setDescription(e.target.value)} required className={inputBase} style={inputStyle} />
          </div>
          <div>
          ...
          <div className="flex gap-2 pt-1">
            <button type="submit" disabled={saving} data-testid="save-income-btn"
              className="flex-1 py-3 rounded-xl text-sm font-semibold active:scale-[0.98] transition-all disabled:opacity-50"
              style={{ background: '#1D1D1F', color: '#FFFFFF', ...font }}>{saving ? 'Saving...' : 'Save Income'}</button>
